...
public class VariableExample2 {
	...
	public static void main(String[] args) {
		...
		booleanData = true; // booleanData 변수에 true 값을 집어 넣음
	...
	// 자료형  변수명 = 리터럴;
		// 프로그래밍에서는 다입되는 이 데이터(값 자체)를 리터럴이라는 단어로도 표현합니다
		...
		int int_Number; // 자바는 캄벨표기법 사용
						// _ 작성 표기법은 DB에서 사용
						// MEMBER_NAME
		// 5. 카멜표기법
		// 변수명 작성 시 여러 단어를 이어서 작성하는 경우
		// 띄어쓰지 않고 후속 단어 첫 글자를 대문자로 작성
		char hellowWorldAppleBananaTomato;
		
		// 6. 변수명은 언어를 가리지 않음(하지만 영어로 씀)
		int 정수1번;
		double 실수2번 = 3.14;
		...
		//PI_VALUE = 1.25252; // 에러 재대입 불가 
		
		/*
		 * 상수(항상 같은 수) :
		 * - 변수의 한 종류
		 * - 한 번 값이 대입되면 다른 값을 재대입할 수 없음
		 * - 자료형 앞에 final이라는 값을 작성
		 * 
		 * -상수의 명령 규칙 : 모두 대문자로 쓰고 여러 단어 작성 시_ 사용
		 * 
		 * - 상수를 사용하는 경우
